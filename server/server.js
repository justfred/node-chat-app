...
io.on('connection', (socket) => {
  ...
  socket.on('join', (params, callback) => {
    ...
    //remove user in case they were there
    users.removeUser(socket.id);
    //socket.leave(params.room)

    // io.emit - every connected user
    // io.to(room)
    // socket.broadcast.emit - everyone connected except the sender
    ...
    socket.broadcast.to(params.room).emit('newMessage', generateMessage('Admin', `New user joined ${params.room}`));
